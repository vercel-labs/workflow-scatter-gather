...
export type ScatterGatherCodeWorkbenchProps = {
  workflowCode: string;
...
function CodePane({
  ...
  useEffect(() => {
    if (gutterMarkMap.size === 0 && activeLines.length === 0) {
  ...
  }, [activeLines.length, gutterMarkMap]);
...
export function ScatterGatherCodeWorkbench({
  workflowCode,
  workflowLinesHtml,
  workflowActiveLines,
  workflowGutterMarks,
  ...
  stepGutterMarks,
  tone,
}: ScatterGatherCodeWorkbenchProps) {
  return (
    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
      <CodePane
        code={workflowCode}
        filename="workflows/scatter-gather.ts"
